...
impl Renderer {
    ...
    pub fn initialize<T: TextureMap>(
        ...
        let vbo_handle = Some(unsafe { gl.create_buffer() }.map_err(InitError::CreateBufferObject)?);
        let ebo_handle = Some(unsafe { gl.create_buffer() }.map_err(InitError::CreateBufferObject)?);
    ...
    fn render_elements<T: TextureMap>(
        ...
        if clip_x1 >= fb_width || clip_y1 >= fb_height || clip_x2 < 0.0 || clip_y2 < 0.0 || clip_x2 <= clip_x1 || clip_y2 <= clip_y1 {
            return;
        }
...
impl TextureMap for SimpleTextureMap {
    ...
    fn gl_texture(&self, imgui_texture: imgui::TextureId) -> Option<glow::Texture> {
        ...
        NonZeroU32::new(imgui_texture.id() as u32)
...
impl GlStateBackup {
    ...
    fn post_init(&mut self, gl: &Context) {
        ...
        unsafe {
            gl.bind_texture(glow::TEXTURE_2D, NonZeroU32::new(self.texture as u32).map(glow::NativeTexture));
        }
    }
    ...
    fn post_render(&mut self, gl: &Context, _gl_version: GlVersion) {
        ...
        unsafe {
            ...
            gl.bind_texture(glow::TEXTURE_2D, NonZeroU32::new(self.texture as _).map(glow::NativeTexture));
            #[cfg(feature = "bind_sampler_support")]
            ...
            gl.bind_buffer(glow::ARRAY_BUFFER, NonZeroU32::new(self.array_buffer as _).map(glow::NativeBuffer));
